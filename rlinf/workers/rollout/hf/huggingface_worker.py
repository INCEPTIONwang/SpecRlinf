...
class MultiStepRolloutWorker(Worker):
    ...
    async def evaluate(self, input_channel: Channel, output_channel: Channel):
        ...
        timing_metric_buffers = {
            "preprocess_ms": [None for _ in range(self.num_pipeline_stages)],
            "vlm_forward_ms": [None for _ in range(self.num_pipeline_stages)],
            "prefill_ms": [None for _ in range(self.num_pipeline_stages)],
            "prefill_total_ms": [None for _ in range(self.num_pipeline_stages)],
            "diffusion_ms": [None for _ in range(self.num_pipeline_stages)],
            "diffusion_step_prep_ms": [None for _ in range(self.num_pipeline_stages)],
            "diffusion_suffix_prep_ms": [None for _ in range(self.num_pipeline_stages)],
            "diffusion_suffix_forward_ms": [None for _ in range(self.num_pipeline_stages)],
            "diffusion_action_proj_ms": [None for _ in range(self.num_pipeline_stages)],
            "diffusion_denoise_math_ms": [None for _ in range(self.num_pipeline_stages)],
            "diffusion_step_update_ms": [None for _ in range(self.num_pipeline_stages)],
            "verify_prefill_ms": [None for _ in range(self.num_pipeline_stages)],
            "verify_diffusion_ms": [None for _ in range(self.num_pipeline_stages)],
            "verify_check_ms": [None for _ in range(self.num_pipeline_stages)],
            "verify_total_ms": [None for _ in range(self.num_pipeline_stages)],
            "verify_calls": [None for _ in range(self.num_pipeline_stages)],
            "draft_total_ms": [None for _ in range(self.num_pipeline_stages)],
            "total_decode_ms": [None for _ in range(self.num_pipeline_stages)],
        }
        spec_total_accept = 0
        ...
        def _init_timing_buffers(stage_id: int, num_envs: int):
            for stage_buffers in timing_metric_buffers.values():
                if stage_buffers[stage_id] is not None and len(stage_buffers[stage_id]) == int(num_envs):
                    continue
                stage_buffers[stage_id] = [[] for _ in range(num_envs)]

        def _record_timing_info(
            stage_id: int,
            timing_info_list: list[dict[str, Any]] | dict[str, Any] | None,
            env_indices: list[int] | None,
            num_envs: int,
        ):
            if isinstance(timing_info_list, dict):
                timing_info_list = [timing_info_list]
            if not isinstance(timing_info_list, list) or not timing_info_list:
                return
            _init_timing_buffers(stage_id, num_envs)
            if env_indices is None:
                env_indices = list(range(len(timing_info_list)))
            for local_idx, info in enumerate(timing_info_list):
                if not isinstance(info, dict):
                    continue
                if local_idx >= len(env_indices):
                    continue
                env_idx = int(env_indices[local_idx])
                for key, stage_buffers in timing_metric_buffers.items():
                    value = info.get(key)
                    if value is None:
                        continue
                    try:
                        stage_buffers[stage_id][env_idx].append(float(value))
                    except Exception:
                        continue
        ...
        def _record_spec_info(
            ...
            _record_timing_info(stage_id, spec_info_list, env_indices, num_envs)
            if env_indices is None:
        ...
        def _log_eval_info(stage_id: int, eval_info: list[dict[str, Any] | None]):
            ...
            for env_idx, info in enumerate(eval_info):
                ...
                timing_call_count = 0
                timing_avgs: dict[str, float] = {}
                for key, stage_buffers in timing_metric_buffers.items():
                    env_values = []
                    if stage_buffers[stage_id] is not None:
                        env_values = stage_buffers[stage_id][env_idx]
                    timing_call_count = max(timing_call_count, len(env_values))
                    timing_avgs[key] = _mean_or_nan(env_values)
                if timing_call_count > 0:
                    timing_line = (
                        f"{prefix}_timing episode={int(info['episode'])} "
                        f"calls={timing_call_count} "
                        f"preprocess_ms={timing_avgs['preprocess_ms']:.3f} "
                        f"vlm_forward_ms={timing_avgs['vlm_forward_ms']:.3f} "
                        f"prefill_ms={timing_avgs['prefill_ms']:.3f} "
                        f"prefill_total_ms={timing_avgs['prefill_total_ms']:.3f} "
                        f"diffusion_ms={timing_avgs['diffusion_ms']:.3f} "
                        f"diffusion_step_prep_ms={timing_avgs['diffusion_step_prep_ms']:.3f} "
                        f"diffusion_suffix_prep_ms={timing_avgs['diffusion_suffix_prep_ms']:.3f} "
                        f"diffusion_suffix_forward_ms={timing_avgs['diffusion_suffix_forward_ms']:.3f} "
                        f"diffusion_action_proj_ms={timing_avgs['diffusion_action_proj_ms']:.3f} "
                        f"diffusion_denoise_math_ms={timing_avgs['diffusion_denoise_math_ms']:.3f} "
                        f"diffusion_step_update_ms={timing_avgs['diffusion_step_update_ms']:.3f} "
                        f"verify_prefill_ms={timing_avgs['verify_prefill_ms']:.3f} "
                        f"verify_diffusion_ms={timing_avgs['verify_diffusion_ms']:.3f} "
                        f"verify_check_ms={timing_avgs['verify_check_ms']:.3f} "
                        f"verify_total_ms={timing_avgs['verify_total_ms']:.3f} "
                        f"verify_calls={timing_avgs['verify_calls']:.3f} "
                        f"draft_total_ms={timing_avgs['draft_total_ms']:.3f} "
                        f"total_decode_ms={timing_avgs['total_decode_ms']:.3f}"
                    )
                    self._append_spec_log(timing_line)
                    for stage_buffers in timing_metric_buffers.values():
                        if stage_buffers[stage_id] is not None:
                            stage_buffers[stage_id][env_idx] = []
            ...
            for _ in range(n_chunk_steps):
                for stage_id in range(self.num_pipeline_stages):
                    ...
                    if need_plan:
                        ...
                        timing_info_list = (
                            result.get("timing_info") if isinstance(result, dict) else None
                        )
