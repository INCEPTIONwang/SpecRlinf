...
class GR00T_N1_5_ForRLActionPrediction(GR00T_N1_5, BasePolicy):
    ...
    def _spec_action_horizon(self) -> int:
        ...
        if spec_action_horizon > action_horizon:
            raise ValueError(
                f"spec_action_horizon={spec_action_horizon} must be <= action_horizon={action_horizon}"
            )
    ...
    def _spec_sample_actions(
        ...
        *,
        ...
        state_features = self.action_head.state_encoder(action_inputs.state, embodiment_id)
    ...
    def _speculative_decode_chunk(
        ...
        *,
        ...
        obs_batch = self._repeat_normalized_input(normalized_input, int(batch_size))
        ...
        def _run_verification_chunk(
            *,
            ...
            for i in range(int(verify_order.shape[0])):
                ...
                draft_vec = np.asarray(greedy_chain_raw_full[pos])
                ...
                if spec_debug:
                    self._append_spec_log(
                        "gr00t_spec_reject "
                        f"kind={kind} pos={int(pos)} rank={int(i)} conf={conf_pos:.4f} "
                        f"chunk_start={int(chunk_start)} abs_diff_max={float(np.max(abs_diff)) if abs_diff.size else float('nan'):.4f} "
                        f"over_dims={over.astype(np.int8).tolist()}"
                    )
        ...
        for order_conf_full, order_seq_full, chunk_start, chunk_end in chunk_orders:
            ...
            obs_verify = self._repeat_normalized_input(normalized_input, k_total)
            fixed_actions_t = torch.as_tensor(
                fixed_actions_batch, device=obs_verify["state"].device
            )
            fixed_action_mask_t = torch.as_tensor(
                fixed_action_mask_batch, device=obs_verify["state"].device
            )
