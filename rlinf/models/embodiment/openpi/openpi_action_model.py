...
class OpenPi0ForRLActionPrediction(PI0Pytorch, BasePolicy):
    ...
    def _compute_confidence(
        ...
        high = conf >= tau[None, :]
        return conf.astype(np.float32), tau.astype(np.float32), high

    def _select_draft(self, actions: np.ndarray, *, alpha: float, eps: float, conf_dim: int) -> dict[str, Any]:
        conf, tau, high = self._compute_confidence(actions, alpha=alpha, eps=eps, conf_dim=conf_dim)
    ...
    def _generate_chunked_draft(
        ...
        *,
        ...
        prefix_horizon = int(min(5, total_horizon)) if total_horizon > 0 else 5
        obs_b = self._repeat_observation(observation, batch_size)
        device = observation.state.device

        actions_prefix_raw = self._spec_sample_actions(
            obs_b,
            num_steps=int(diffusion_num_steps),
            action_horizon=int(prefix_horizon),
            capture_prefix_cache=True,
            profile_timing=bool(timing_enabled),
        )
        prefix_cache = getattr(self, "last_prefix_cache", None)
        actions_prefix_exec = self._apply_output_transform(
            actions_prefix_raw, obs_b.state, limit_chunk=False
        )

        timing_info = {}
        if timing_enabled:
            timing_info["prefill_ms"] = float(getattr(self, "last_timing_prefill_ms", 0.0))
            timing_info["diffusion_ms_prefix"] = float(getattr(self, "last_timing_diffusion_ms", 0.0))

        if not chunked or int(prefix_horizon) >= int(total_horizon):
            fixed_actions = torch.zeros(
                (int(batch_size), int(total_horizon), int(actions_prefix_raw.shape[2])),
                dtype=actions_prefix_raw.dtype,
                device=actions_prefix_raw.device,
            )
            fixed_action_mask = torch.zeros(
                (int(batch_size), int(total_horizon)),
                dtype=torch.bool,
                device=actions_prefix_raw.device,
            )
            fixed_actions[:, :prefix_horizon] = actions_prefix_raw[:, :prefix_horizon]
            fixed_action_mask[:, :prefix_horizon] = True
            ...
            if timing_enabled:
                timing_info["diffusion_ms_full"] = float(getattr(self, "last_timing_diffusion_ms", 0.0))
            return actions_full_exec, actions_full_raw.detach().cpu().numpy(), prefix_cache, timing_info

        seg = int(max(1, rollout_segment_size))
        curr_h = int(prefix_horizon)
        actions_raw_full = actions_prefix_raw
        actions_exec_full = actions_prefix_exec

        while curr_h < int(total_horizon):
            next_h = int(min(curr_h + seg, int(total_horizon)))
            fixed_actions = torch.zeros(
                (int(batch_size), int(next_h), int(actions_raw_full.shape[2])),
                dtype=actions_raw_full.dtype,
                device=actions_raw_full.device,
            )
            fixed_action_mask = torch.zeros(
                (int(batch_size), int(next_h)),
                dtype=torch.bool,
                device=actions_raw_full.device,
            )
            fixed_actions[:, :curr_h] = actions_raw_full[:, :curr_h]
            fixed_action_mask[:, :curr_h] = True
            actions_raw_full = self._spec_sample_actions(
                obs_b,
                num_steps=int(diffusion_num_steps),
                action_horizon=int(next_h),
                fixed_actions=fixed_actions,
                fixed_action_mask=fixed_action_mask,
                prefix_cache=prefix_cache,
                profile_timing=bool(timing_enabled),
            )
            actions_exec_full = self._apply_output_transform(
                actions_raw_full, obs_b.state, limit_chunk=False
            )
            curr_h = int(next_h)
            if timing_enabled:
                timing_info.setdefault("diffusion_ms_steps", 0.0)
                timing_info["diffusion_ms_steps"] += float(getattr(self, "last_timing_diffusion_ms", 0.0))

        return actions_exec_full, actions_raw_full.detach().cpu().numpy(), prefix_cache, timing_info
    ...
    def _speculative_decode_chunk(
        ...
        *,
        ...
        env_dim = int(getattr(self.config, "action_env_dim", actions_raw_full.shape[-1]))
        env_dim = max(1, min(env_dim, int(actions_raw_full.shape[-1])))
        actions_raw_compare = actions_raw_full[..., :env_dim]
        ...
        greedy_chain_raw_compare = np.zeros((h, env_dim), dtype=actions_raw_compare.dtype)
        greedy_chain_raw_compare[0] = actions_raw_compare[best_first_idx, 0]
        for t in range(1, h):
            greedy_chain_exec[t] = actions_exec[selected_path, t]
            greedy_chain_raw_full[t] = actions_raw_full[selected_path, t]
            greedy_chain_raw_compare[t] = actions_raw_compare[selected_path, t]
        ...
        verify_order_chunks_conf: list[np.ndarray] = []
        verify_order_chunks_seq: list[np.ndarray] = []
        chunk_summaries: list[dict[str, Any]] = []
        chunk_orders: list[tuple[np.ndarray, np.ndarray, int, int]] = []
        for start in range(1, h, chunk_size):
            end = min(start + chunk_size, h)
            pos = np.arange(start, end, dtype=np.int64)
            order_conf = pos[np.lexsort((pos, -draft_conf_per_t[pos]))]
            order_seq = pos
            verify_order_chunks_conf.append(order_conf)
            verify_order_chunks_seq.append(order_seq)
        ...
        def _run_verification_chunk(
            *,
            ...
            for i in range(int(verify_order.shape[0])):
                ...
                pred_vec = np.asarray(verify_actions_compare_slice[i, pos])
                draft_vec = np.asarray(greedy_chain_raw_compare[pos])
        ...
        accepted_positions_conf: set[int] = {0}
        accepted_positions_seq: set[int] = {0}
        accepted_rank_conf = 0
        accepted_rank_seq = 0
        ...
        for order_conf, order_seq, chunk_start, _chunk_end in chunk_orders:
            verify_h = int(h)
            k_conf = int(order_conf.shape[0]) if conf_active else 0
            k_seq = int(order_seq.shape[0]) if seq_active else 0
            ...
            verify_actions_raw_np = verify_actions_raw.detach().cpu().numpy()
            verify_actions_compare = verify_actions_raw_np[..., :env_dim]
            ...
            if conf_active:
                (
                    ...
                    verify_actions_compare_slice=verify_actions_compare[:k_conf],
